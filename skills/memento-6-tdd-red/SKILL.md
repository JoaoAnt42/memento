...
3. Run each `check` in the plan's `## Checks` before implementation, in the worktree of the repo the entry is tagged for, and record its failing output under that entry in the plan. Stop when a criterion's check id is missing from `## Checks`, and when two entries share an id. `observe` entries don't run here — they only work after deploy.
...
- **Tests are grouped by owning task** — the `(T<k>)` on each criterion line — so a task's brief is the criteria it turns green. A task with no criterion test gets its `## Data contract` seam test, so every task still hands step 7 a red test. A task with neither → stop and loop back to `memento-2-planning` to merge it; never write a test with no criterion behind it.
...
- **A `check` that can't run here is an `observe`.** A command needing deploy, credentials the agent lacks, or shared infrastructure was classified wrong at planning; reclassifying a `check` as `observe` stops the cycle and follows **Re-scope after approval** in `memento-4-human-review`. Neither kind sets `needs_human_smoke`.
