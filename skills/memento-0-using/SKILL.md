...
- **Verdict (step 4)** — **hard stop.** Run 0a→3, write the plan, set `status: auto-review`, and hand back with the plan path plus the unilateral-decisions list. Never self-approve, never create worktrees, never post acceptance criteria, never proceed past step 4 — neither step 5 nor step 6. The human approves later from their own session.
...
- **Derive** a conventional-commit `type`, a kebab-case `slug`, and the target repo (root path + its integration branch) from the task, and read the ticket's body and existing acceptance criteria when the task names one.
- **Write a minimal plan file** at `~/Documents/be_JLA/work/plans/<repo>/YYYY-MM-DD-<slug>.md`, `<repo>` being `basename` of the target repo root (create the subfolder if absent) — frontmatter (`name`, `status: implementing`, `created`, `type`, `needs_human_smoke: false`, and a one-element `repos:` list — `label: repo`, `path` = the target repo root, `base` = its integration branch, `branch: <type>/<slug>`, empty `worktree:`) plus a short `## Context` paragraph and `## Acceptance criteria` in `memento-2-planning`'s format, without `(T<k>)` since there are no tasks, plus `## Checks` when a criterion uses `check` or `observe`. No Decisions / Tasks / Open-risks sections. Criteria are never posted to the ticket on this route; the drafted ones go in the reply. This file is what `memento-8` reads the worktree from; it is deleted at merge (step 9), so it is a coordination artifact for the cycle, not an archive.
...
- **Implement directly** — normal editing, no agent dispatch, no red-SHA hand-off. Honour CLAUDE.md TDD where it fits, but no `memento-6` / `memento-7` ceremony. Write each criterion's test id into `## Acceptance criteria` as you go; a criterion only a human smoke test can verify makes the task Large, so restart it. A `check` or `observe` criterion keeps the task Small — neither sets `needs_human_smoke: true`. The Small route has no steps 6 and 7, so run each `check` red before implementing and green after, and record its red and green output under its `## Checks` entry in the plan; `observe` entries only run after deploy.
- Invoke `memento-8-final-review` — parallel reviewers + Orchestrator reconciliation, opens the PR. It hands on to `memento-8c-work-summary` for the shareable summary, which moves the plan to `status: in-review` once CI has a verdict. Step 9 deletes the plan when the PR merges.
...
6. `memento-6-tdd-red` — test-writer agents produce red tests (one verifier per acceptance criterion, grouped by owning task), **commit the red SHA**
...
8.6. `memento-8c-work-summary` — emits the shareable one-line summary (issue URL - PR URL - what the work was for) once the PRs are open. Never skipped, never asked for.
9. `memento-9-receiving-review` — consume review feedback with technical rigor, not performative agreement

## Rules

- **Plan file coordinates the live cycle; it is not the archive.** Lives at `~/Documents/be_JLA/work/plans/<repo>/YYYY-MM-DD-<slug>.md` — one subfolder per target repo, named `basename(repos[0].path)`; a multi-repo plan sits under its first entry only. Frontmatter `status:` tracks current step. Appended across steps, never overwritten. Small writes a minimal version (frontmatter + Context + Acceptance criteria); Large writes the full structure. It exists to hand state between steps (worktree paths, red SHA, decisions later steps depend on) — **the durable record of what shipped is the PR body (step 8), and the plan is deleted when the PR merges (step 9).** The one exception is a task that never merged (abandoned) or grew a post-merge life (revert/rollout/runbook): see step 9's graduation exception.
