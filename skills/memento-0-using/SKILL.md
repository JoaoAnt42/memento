...
- **Implement directly** — normal editing, no agent dispatch, no red-SHA hand-off. Honour CLAUDE.md TDD where it fits, but no `memento-6` / `memento-7` ceremony. Write each criterion's test id into `## Acceptance criteria` as you go; a criterion only a human smoke test can verify makes the task Large, so restart it.
