...
4. **Tests frozen — enforce mechanically.** Tests must be byte-identical to the repo's last red SHA, not the task's own, since tasks sharing a test file add red commits after an earlier task's: `git diff --quiet <last-red-SHA> HEAD -- <task test paths>` must exit clean. Any diff to a test file (or to fixtures/conftest under those paths) is a reject — the implementer may not make tests pass by editing them.
...
- Tests are immutable during step 7 — enforced by `git diff --quiet <last-red-SHA> HEAD -- <test paths>`. If tests are wrong, loop back to step 6; if the criterion is wrong, not the test, loop back to `memento-2-planning` to re-scope it.
