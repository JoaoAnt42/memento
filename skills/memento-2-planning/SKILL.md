...
Posted: <YYYY-MM-DD> body (AC…), comment (AC…)[; <next post>…]   <!-- step 4 appends one entry per post, never rewrites one; absent until the first post -->

- AC<n> [ticket] <text verbatim> → <verifier> (T<k>)
- AC<n> [drafted] <text> → <verifier> (T<k>)
- AC<n> [re-scoped: <why>, <YYYY-MM-DD>] <new text> (was: <old text>) → <verifier> (T<k>)
