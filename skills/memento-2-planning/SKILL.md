...
## Acceptance criteria
Source: <ticket URL>[, <ticket URL>…] | <epic URL> (epic) | none
Posted: <YYYY-MM-DD> <ticket> body (AC4, AC5); <YYYY-MM-DD> <ticket> comment (AC3 r2)   <!-- step 4 appends one entry per successful write; absent until the first -->

- AC<n> [ticket] <text verbatim> → <verifier> (T<k>)
- AC<n> [drafted] <text> → <verifier> (T<k>)
- AC<n> [re-scoped: <why>, r<n>] <new text> (was: <old text>) → <verifier> (T<k>)

## Checks   <!-- omit when no criterion uses check or observe -->
- <id> [repo: <label>] — `<command>` → expect <expectation>
  - Red <YYYY-MM-DD>: <observed output>          <!-- step 6, or the Small route -->
  - Green <YYYY-MM-DD> at <sha>: <observed output>
- C1 — `curl -s localhost:8080/healthz` → expect `{"status":"ok"}`
...
- **The plan owns the acceptance criteria.** Write `## Acceptance criteria` once research settles: the ticket's criteria verbatim as `[ticket]`, missing ones as `[drafted]` per the `write-ticket` skill's acceptance-criteria rules, unhappy path included. A changed criterion is marked `[re-scoped: <why>, r<n>]` (`r1`, then +1 on each re-scope of it), never changed silently. `<verifier>` is `test` (step 6 swaps in the test id), `check <id>`, `observe <id>`, or `smoke`, and any `smoke` verifier forces `needs_human_smoke: true`; with several tickets, each line names its ticket. After step 4, a wrong or missing criterion follows **Re-scope after approval** in `memento-4-human-review`.
- **`check` and `observe` are commands, not tests.** Each points at a `## Checks` entry by id — `→ check C1 (T2)` runs before merge, `→ observe C3 (T4)` only after deploy. Ids run C1, C2, …, unique per plan. The entry carries the command and the output it must produce; the kind lives on the criterion line. Tag an entry `[repo: <label>]` on a multi-repo plan — it picks which PR body an `observe` entry reaches; a single-repo plan can drop the tag. An expectation is an exact string or regex match against the command's output, and it comes from running the command — never from a figure quoted in a ticket or plan. A criterion gets `check` only when its command runs in the worktree before implementation, deterministically, without credentials the agent lacks and without side effects on shared infrastructure; everything else is `observe`. A check command finishes in about two minutes and never waits on input; one that can't is an `observe` or a `test`. Neither sets `needs_human_smoke`.
- **Tasks are atomic.** Each task must be independently testable so steps 6–8 can dispatch independent agents. Each acceptance-criterion test belongs to the task that turns it green, recorded only as `(T<k>)` on the criterion's line; when several tasks touch a criterion, that is the task whose change makes it pass (the later one in task order). A task that turns no criterion green gets its `## Data contract` seam test; a task with neither is merged into a neighbour at planning.
