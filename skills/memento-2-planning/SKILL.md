...
Posted: <YYYY-MM-DD> body, comment   <!-- step 4 writes this after each post; absent until then -->

- AC<n> [ticket] <text verbatim> → <verifier> (T<k>)
- AC<n> [drafted] <text> → <verifier> (T<k>)
- AC<n> [re-scoped: <why>] <new text> (was: <old text>) → <verifier> (T<k>)
...
- **The plan owns the acceptance criteria.** Write `## Acceptance criteria` once research settles: the ticket's criteria verbatim as `[ticket]`, missing ones as `[drafted]` per the `write-ticket` skill's acceptance-criteria rules, unhappy path included. A changed criterion is marked `[re-scoped: <why>]`, never changed silently. `<verifier>` is `test` (step 6 swaps in the test id) or `smoke`; with several tickets, each line names its ticket. A later step (5, 7, 7b, 9) that finds a criterion wrong loops back here to re-scope it, and the plan goes through step 4 again, which posts the change.
- **Tasks are atomic.** Each task must be independently testable so steps 6–8 can dispatch independent agents. Each acceptance-criterion test belongs to the task that turns it green (the last in its chain), recorded only as `(T<k>)` on the criterion's line. A task that turns no criterion green gets its `## Data contract` seam test; a task with neither is merged into a neighbour.
